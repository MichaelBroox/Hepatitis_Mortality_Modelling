...
def plot_model_summary(
    ...
    ax.set_xlabel('Model', fontsize=22, labelpad=20)
    
    plt.xticks(rotation=20)
    
    plt.yticks(fontsize=25)
    ax.set_ylabel('Accuracy', fontsize=22, labelpad=20)
...
def evaluate_classifier(
    ...
    print("#############################################################")
    print(f"Classifier's Test Accuracy : {round(test_accuracy_score, 4)}")
    ...
    sns.heatmap(
        ...
        cbar_kws={
            'label':'', 
            'orientation': cm_bar_orientation, 
            'shrink':0.4,
        },
        cmap=cmap, 
        fmt='d', 
    ...
    ax.text(2.25, -0.10,'Accuracy: '+ str(round(test_accuracy_score, 4)), fontsize=14)
